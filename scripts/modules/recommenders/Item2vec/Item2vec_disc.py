...
class Seq2Vec(Item2vec_abstract):
    ...
    def __init__(self, embedding_dir, factors=100, w_size=-1, learning_rate=0.25, 
                 ...
                 regularization=-1, recomender_norm=True, big_innit=False, time_threshold=1800):
        ...
        self.time_threshold = time_threshold * 86400
    ...
    def timestamp_diff(self, df):
        ...
        # 2. Create Session Splits 
        df['mask'] = df[kw.COLUMN_TIME_DIFF] >= self.time_threshold
        df['increment'] = df.groupby(kw.COLUMN_USER_ID)['mask'].cumsum()
        
        # 3. Create Pseudo-User IDs
