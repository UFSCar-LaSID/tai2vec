...
class Item2vec_temp_model:
    def __init__(self, embedding_dir, factors=50, w_size=-1, learning_rate=0.25, min_learning_rate = 0.0025 ,subsample = 0.001, batch_size = kw.MEM_SIZE_LIMIT, negative_samples=3, negative_exp=0.75, epochs=100, lr_decay=0.1, time_exp=1.5, min_time_diff=300):
    ...
    def fit(self, df):
        ...
        #self.model.compile(optimizer= Adam(learning_rate=self.learning_rate),loss=tf.keras.losses.BinaryCrossentropy(from_logits=False))
